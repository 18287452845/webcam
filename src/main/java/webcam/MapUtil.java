...
public class MapUtil {
	...
	public static String glass(Map<String, Double> eye) {
		...
		// 查找值大于90.0的条目
		return eye.entrySet().stream()
				.filter(entry -> entry.getValue() > 90.0)
				.findFirst()
				.map(Entry::getKey)
				.map(MapUtil::getEyeStatusDescription)
				.orElse("无法识别");
	}
	 ...
	 * @return 状态描述
	 */
	private static String getEyeStatusDescription(String key) {
		return switch (key) {
			case "rightNormalOpen", "leftNormalOpen" -> "戴眼镜并且睁眼";
			case "rightNormalClose", "leftNormalClose" -> "戴眼镜并且闭眼";
			case "rightNoOpen", "leftNoOpen" -> "不带眼镜并且睁眼";
			case "leftNoClose", "rightNoClose" -> "不带眼镜并且闭眼";
			case "rightDark", "leftDark" -> "戴墨镜";
			case "rightOcclusion", "leftOcclusion" -> "眼镜前面有遮挡物";
			default -> "无法识别";
		};
	}
	...
	public static String gender(String value) {
		return "Male".equals(value) ? "男性" : "女性";
	}
